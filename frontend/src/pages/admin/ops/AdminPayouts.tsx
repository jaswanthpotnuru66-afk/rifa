        ...
        <AdminOpsLayout>
            <div className="space-y-8 animate-in fade-in duration-500 pb-24">
                ...
                <div className="space-y-4">
                    {/* Header */}
                    <div className="hidden lg:grid grid-cols-[100px_1fr_100px_80px_100px_100px_100px_100px_120px_100px_120px_40px] gap-4 px-6 py-4 text-[9px] font-black uppercase tracking-widest text-neutral-400 items-center border border-transparent">
                        <div>ID</div>
                        <div>Maker</div>
                        <div>Date</div>
                        <div className="lg:flex lg:justify-center w-full">Orders</div>
                        ...
                        <div className="lg:flex lg:justify-end w-full">Shipping</div>
                        <div className="lg:flex lg:justify-end w-full">Net Paid</div>
                        <div className="lg:flex lg:justify-center w-full">Status</div>
                        <div className="lg:flex lg:justify-center w-full">Actions</div>
                        <div></div>
                    </div>
                            ...
                            >
                                <div className="flex flex-col lg:grid lg:grid-cols-[100px_1fr_100px_80px_100px_100px_100px_100px_120px_100px_120px_40px] gap-4 p-4 lg:p-6 items-center">
                                    ...
                                    <div className="flex justify-between lg:block w-full lg:w-auto">
                                        ...
                                        <div className="text-[10px] font-black text-neutral-400 font-inter">{payout.id}</div>
                                    </div>
                                    ...
                                    <div className="flex justify-between lg:block w-full lg:w-auto">
                                        ...
                                        <div className="text-right lg:text-left">
                                            ...
                                            <Link to={`/admin/ops/makers/${payout.makerId}`} onClick={(e) => e.stopPropagation()} className="text-[9px] font-black uppercase tracking-widest text-neutral-300 hover:text-brand-pink">View Profile</Link>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex justify-between lg:flex w-full lg:w-auto">
                                        ...
                                        <div className="text-[10px] font-medium text-neutral-500 font-inter">{new Date(payout.date).toLocaleDateString()}</div>
                                    </div>

                                    <div className="flex justify-between lg:flex lg:justify-center w-full lg:w-auto">
                                        <span className="lg:hidden text-[9px] font-black uppercase tracking-widest text-neutral-400">Orders</span>
                                        <div className="text-xs font-bold text-neutral-600 font-inter">{payout.orderCount}</div>
                                    </div>
                                    ...
                                    <div className="flex justify-between lg:flex lg:justify-end w-full">
                                        ...
                                        <div className="text-xs font-bold text-neutral-950 font-inter">₹{payout.grossAmount.toLocaleString()}</div>
                                    </div>
                                    
                                    <div className="flex justify-between lg:flex lg:justify-end w-full">
                                        <span className="lg:hidden text-[9px] font-black uppercase tracking-widest text-neutral-400">Comm.</span>
                                        <div className="text-xs font-medium text-neutral-400 italic font-inter">−₹{payout.commission.toLocaleString()}</div>
                                    </div>
                                    
                                    <div className="flex justify-between lg:flex lg:justify-end w-full">
                                        <span className="lg:hidden text-[9px] font-black uppercase tracking-widest text-neutral-400">TCS</span>
                                        <div className="text-xs font-medium text-neutral-400 italic font-inter">−₹{payout.tcs.toLocaleString()}</div>
                                    </div>
                                    
                                    <div className="flex justify-between lg:flex lg:justify-end w-full">
                                        <span className="lg:hidden text-[9px] font-black uppercase tracking-widest text-neutral-400">Shipping</span>
                                        <div className="text-xs font-black text-red-600 font-inter">
                                    ...
                                    <div className="flex justify-between lg:flex lg:justify-end pt-3 lg:pt-0 border-t lg:border-0 border-neutral-50 mt-2 lg:mt-0 w-full">
                                        ...
                                        <div className="text-sm font-black text-brand-pink font-inter">₹{payout.netPaid.toLocaleString()}</div>
                                    </div>
                                    ...
                                    <div className="flex justify-between lg:block items-center w-full lg:w-auto">
                                        ...
                                        <div className="flex lg:inline-flex justify-center lg:w-full gap-2" onClick={(e) => e.stopPropagation()}>
                                            {payout.status === 'pending' && (
                                                <>
                                                    <button className="p-2 border border-green-200 text-green-600 hover:bg-green-50 rounded-sm transition-all" title="Approve">
                                                        <CheckCircle2 size={14} />
                                                    </button>
                                                    <button className="p-2 border border-amber-200 text-amber-600 hover:bg-amber-50 rounded-sm transition-all" title="Hold">
                                                        <Ban size={14} />
                                                    </button>
                                                </>
                                            )}
                                    ...
                                    <div className="lg:hidden w-full py-2 bg-neutral-50 text-[9px] font-black uppercase tracking-widest text-neutral-400 flex items-center justify-center gap-2 rounded-sm mt-2">
                                        {expandedPayout === payout.id ? 'Hide Details' : 'View Details'}
                                        <ChevronDown size={12} className={`transition-transform duration-300 ${expandedPayout === payout.id ? 'rotate-180' : ''}`} />
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <tbody className="divide-y divide-neutral-50">
                                                {mockAllOrders.slice(0, 3).map(order => (
                                                    <tr key={order.id} className="text-[10px] font-medium text-neutral-600">
                                                        <td className="px-4 py-3 font-bold text-brand-pink font-inter">{order.id}</td>
                                                        <td className="px-4 py-3 truncate max-w-[150px]">{order.productName}</td>
                                                        <td className="px-4 py-3">{order.buyerCity}</td>
                                                        <td className="px-4 py-3 text-right font-inter">₹{order.amount.toLocaleString()}</td>
                                                        <td className="px-4 py-3 text-right italic text-neutral-400 font-inter">−₹{order.commission.toLocaleString()}</td>
                                                        <td className="px-4 py-3 text-right italic text-neutral-400 font-inter">−₹{order.tcs.toLocaleString()}</td>
                                                        <td className="px-4 py-3 text-right text-red-500 font-inter">{order.weightAdjustment ? `−₹${Math.abs(order.weightAdjustment)}` : '—'}</td>
                                                        <td className="px-4 py-3 text-right font-bold text-neutral-900 font-inter">₹{order.makerPayout.toLocaleString()}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
