...
import { CheckCircle2, AlertCircle, Download, FileText, Database } from 'lucide-react';
import { motion } from 'framer-motion';
import CraftMakerLayout from '../../layouts/CraftMakerLayout';
import { mockTCSData } from '../../lib/craftmaker';
                        ...
                        <div className="flex gap-5">
                            ...
                            <div className="flex-1 space-y-4">
                                ...
                                <div className="flex gap-3 max-w-sm">
                                    <input type="text" placeholder="15-character GSTIN (e.g. 08ABCDE1234F1Z5)" value={gstin} onChange={e => setGstin(e.target.value.toUpperCase())}
                                    ...
                                    <button onClick={() => gstin.length > 0 && setGstinSaved(true)}
                                        className="px-6 py-3 bg-brand-pink text-white text-[10px] font-black uppercase tracking-[0.3em] hover:bg-brand-pink-dark transition-all">
                                        Save
                                    </button>
                                </div>
                            </div>
                            ...
                            <tbody className="divide-y divide-neutral-50">
                                {mockTCSData.map((row, i) => (
                                    <motion.tr key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.06 }}
                                        ...
                                        <td className="px-6 py-5 text-sm font-medium text-neutral-500">₹{row.grossSales.toLocaleString()}</td>
                                        <td className="px-6 py-5 text-sm font-bold text-red-500">−₹{row.tcsDeducted.toLocaleString()}</td>
                                        <td className="px-6 py-5 text-sm font-black text-neutral-950">₹{row.netAmount.toLocaleString()}</td>
                                        <td className="px-6 py-5">
